...
class User
{
    ...
    function get_user_profile_by_id($id){
        $query = "SELECT user_profile FROM chat_user_table WHERE user_id = $id";

        $statement = $this->connect->prepare($query);

        if($statement->execute())
        {
            $user_profile = $statement->fetch(PDO::FETCH_ASSOC);
        }
    ...
    function get_user_name_by_id($id){
        $query = "SELECT user_name FROM chat_user_table WHERE user_id = $id";
