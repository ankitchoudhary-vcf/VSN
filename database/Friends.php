...
class Friends
{
    ...
    {
        $query = "SELECT * FROM chat_user_table  WHERE user_id !=$request_from_id AND NOT EXISTS (SELECT *  FROM friend_request WHERE user_id = friend_request.request_to_id AND friend_request.request_from_id =$request_from_id)";
    ...
    function getFriendRequest($request_to_id)
    {
        $query = "SELECT * FROM chat_user_table JOIN friend_request ON friend_request.request_from_id = user_id WHERE friend_request.request_to_id = $request_to_id AND friend_request.request_status != 'Accepted'";
    ...
    {
        $query = "UPDATE friend_request SET request_status = '$this->status' WHERE request_from_id = $this->request_to_id AND request_to_id = $this->request_from_id;
                INSERT INTO friend_request(request_from_id, request_to_id, request_status) VALUE (:request_from_id, :request_to_id, :request_status);";   
        $statement = $this->connect->prepare($query);
        $statement->BindParam(':request_from_id', $this->request_from_id);
        $statement->BindParam(':request_to_id', $this->request_to_id);
        $statement->BindParam(':request_status', $this->status);
